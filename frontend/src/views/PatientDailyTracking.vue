  ...
  methods: {
    ...
    async fetchDailyTrackingData(id) {
      ...
      try {
        const token = localStorage.getItem('userToken');
        const headers = token ? { 'x-auth-token': token } : {};
        const response = await axios.get(`http://localhost:3001/api/daily-tracking/patient/${id}`, { headers });
    ...
    async addTrackingData() {
      ...
      try {
        const token = localStorage.getItem('userToken');
        const headers = token ? { 'x-auth-token': token } : {};
        const response = await axios.post('http://localhost:3001/api/daily-tracking', payload, { headers });
