        ...
        <v-row>
          ...
          <v-col cols="12" md="7">
            <v-card class="pa-5 mb-6">
                    ...
                    <v-col cols="12" sm="6" md="6">
                      <v-menu v-model="timeMenu" :close-on-content-click="false" transition="scale-transition" offset-y min-width="290px">
                        <template v-slot:activator="{ on, attrs }">
                          <v-text-field v-model="timePicker" label="เวลานัดหมาย" prepend-icon="mdi-clock" v-bind="attrs" :rules="[v => !!v || 'กรุณาใส่เวลานัดหมาย']" required outlined dense placeholder="HH:MM" />
                        </template>
              ...
              <v-card-actions>
                <v-spacer />
                <v-btn color="#3B5F6D" dark @click="addAppointment" :loading="loading" :disabled="!valid"><v-icon left>mdi-plus</v-icon> เพิ่มการนัดหมาย</v-btn>
              </v-card-actions>
            </v-card>
          </v-col>
  ...
  methods: {
    ...
    async fetchAppointments(id){
      ...
      try{
        const token = localStorage.getItem('userToken');
        const headers = token ? {'x-auth-token':token} : {};
        const res = await axios.get(`${API_APPOINTMENTS}patient/${id}`, { headers });
    ...
    async addAppointment(){
      ...
      try{
        const token = localStorage.getItem('userToken');
        const headers = token ? {'x-auth-token':token} : {};
        await axios.post(`${API_APPOINTMENTS}`, { ...this.newAppointment, patient_id:this.patientId, appointment_date:this.datePicker, appointment_time:this.timePicker }, { headers });
    ...
    async deleteAppointment(id){
      if(!confirm('ยืนยันการลบการนัดหมาย?')) return;
      try{
        const token = localStorage.getItem('userToken');
        const headers = token ? {'x-auth-token':token} : {};
    ...
    resetNewAppointmentForm(){
      this.newAppointment = { hn_number:null, rights:null, reason:null, appointed_by:null, contact_location:null, other_details:null, diagnosis:null, status:'รอนัด' };
    ...
    getStatusColor(status){ return this.chartColors[status] || '#000000'; },
    updateChart(){
    ...
    openExportDialog(){ this.selectedAppointmentId=null; this.exportDialog=true; },
